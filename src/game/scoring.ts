...
export function formatCompactNumber(value: number): string {
	...
	for (const unit of units) {
		...
		if (scaled < 10) return `${sign}${scaled.toFixed(1)}${unit.suffix}`
		if (scaled < 100) return `${sign}${Math.round(scaled)}${unit.suffix}`
		if (scaled < 999.5) return `${sign}${Math.round(scaled)}${unit.suffix}`

		const nextUnitIndex = units.indexOf(unit) - 1
		const nextUnit = units[nextUnitIndex]
		if (nextUnit) {
			const nextScaled = absolute / nextUnit.value
			return `${sign}${nextScaled.toFixed(1)}${nextUnit.suffix}`
		}

		return `${sign}999${unit.suffix}`
	}
