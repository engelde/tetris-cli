...
export function createGameState(
	...
	sharedRandomizer?: RandomizerState,
): GameState {
	const randomizer = sharedRandomizer ?? createRandomizerState()
...
export function gameTick(state: GameState, dt: number): void {
	...
	const wasTouching =
		state.currentPiece !== null && isPieceTouching(state.board, state.currentPiece)
	...
	// Check lock delay
	if (state.currentPiece && isPieceTouching(state.board, state.currentPiece)) {
		state.lockDelay = wasTouching ? state.lockDelay + dt : 0
