...
describe("Board", () => {
	...
	test("clearLines removes every completed row in a multi-line clear", () => {
		const board = createBoard()
		const clearedRows = [
			BUFFER_ROWS + BOARD_HEIGHT - 4,
			BUFFER_ROWS + BOARD_HEIGHT - 3,
			BUFFER_ROWS + BOARD_HEIGHT - 2,
			BUFFER_ROWS + BOARD_HEIGHT - 1,
		]

		for (const row of clearedRows) {
			for (let c = 0; c < BOARD_WIDTH; c++) {
				board[row][c] = { type: "I", color: "cyan" }
			}
		}

		clearLines(board, getCompletedLines(board))

		assert.equal(getCompletedLines(board).length, 0)
		for (const row of clearedRows) {
			assert.ok(
				board[row].every((cell) => cell.type === null),
				`row ${row} should have been cleared`,
			)
		}
	})
